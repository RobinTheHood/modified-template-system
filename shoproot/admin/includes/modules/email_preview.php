...
/* -----------------------------------------------------------------------------------------
  ...
  if ($email_preview) {
    ...
    $html_signatur = '';
    $txt_signatur = '';
    if (file_exists($modCoreTemplate->getPath('mail/'.$order->info['language'].'/signatur.html'))) {
    ...
    if (file_exists($modCoreTemplate->getPath('mail/'.$order->info['language'].'/signatur.txt'))) {
      $shop_content_data = $main->getContentData(EMAIL_SIGNATURE_ID, $order->info['languages_id']);
