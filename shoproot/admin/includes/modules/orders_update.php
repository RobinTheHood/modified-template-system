<?php
 /*-------------------------------------------------------------
   $Id: orders_update.php 13789 2021-11-01 13:23:55Z GTB $
  ...
  if ($order->info['orders_status'] != $status || $comments != '' || $email_preview) {
    ...
    if ($_POST['notify'] == 'on' || $email_preview) {
      ...
      $html_mail = $modCoreTemplate->fetch($smarty, 'admin/mail/'.$order->info['language'].'/change_order_mail.html');
      $txt_mail = $modCoreTemplate->fetch($smarty, 'admin/mail/'.$order->info['language'].'/change_order_mail.txt');
